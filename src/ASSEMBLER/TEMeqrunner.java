	...
	private void yearlyrun(){					
		...
		eqrunner.runcht.cht.prepareDayDrivingData(yrindex, usedatmyr);
			...
			for (int im=0; im<12;im++){
				...
				temcj.getData();			
				plotting.updateMlyBioGraph(yrcnt, im, eqrunner.runcht.cht.getBd());
				plotting.updateMlyPhyGraph(yrcnt, im, eqrunner.runcht.cht.getEd());
				
			}
			plotting.updateYlyBioGraph(yrcnt, eqrunner.runcht.cht.getCd(), eqrunner.runcht.cht.getBd());
			plotting.updateYlyPhyGraph(yrcnt, eqrunner.runcht.cht.getCd(), eqrunner.runcht.cht.getEd());
