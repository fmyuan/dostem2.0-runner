				...
				if (outputyrind >=0) {
					if (cht.getMd().getOutSiteDay()){
						for (int id=0; id<dinmcurr; id++) {
								...
								//cht.getOutbuffer().getEnvoddly()[id].chtid = cht.cd.chtid;
								//EnvDataDly *envoddly = &cht.outbuffer.envoddly[id];
								//envdlyouter.outputCohortEnvVars_dly(envoddly, icalyr, im, id, dstepcnt);
					...
					if (cht.getMd().getOutSiteMonth()){
						...
						//envmlyouter.outputCohortEnvVars_mly(&cht.cd.m_snow, &cht.ed, icalyr, im, mstepcnt);
						//bgcmlyouter.outputCohortBgcVars_mly(&cht.bd, icalyr, im, mstepcnt);
						mstepcnt++;
					}
					...
					if (cht.getMd().getOutSiteYear() && im==11){
						...
						//bgcylyouter.outputCohortBgcVars_yly(&cht.bd, icalyr, ystepcnt);
