...
public class PlotterUpdate {
	...
	public void updateYlyBioGraph(int yrcnt, CohortData cd, BgcData allbd){		//update plots
		...
		if(yrcnt%1 ==0 && BioVariablePlotter.f.isVisible()){
			...
			y1 = allbd.getY_a2v().getIngppall();
		    y2 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_GPPt, 1));		 
			y3 = allbd.getY_a2v().getInnppall();
		    y4 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_INNPPt, 1));		 
			var1plotter.ingnppTP.addPoint(x, y1, y2, y3, y4);
							
			y1 = allbd.getY_a2v().getGppall();
			y2 = allbd.getY_a2v().getNppall();
		    y3 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_NPPt, 1));		 
			var1plotter.gnppTP.addPoint(x, y1, y2, y3);

			y1 = allbd.getY_v2soi().getLtrfalcall();
		    y2 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_NPPt, 1));	// litterfall's target is NPP	 
			var1plotter.ltrfalcTP.addPoint(x, y1, y2);
			
			y1 = allbd.getY_soi2v().getInnuptake();
			var1plotter.innuptakeTP.addPoint(x, y1);
			
			y1 = allbd.getY_soi2v().getSnuptakeall()+allbd.getY_soi2v().getLnuptake();
		    y2 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_NUPTAKEt, 1));	 
			var1plotter.nuptakeTP.addPoint(x, y1, y2);

			// The following are for ALL pfts
			y1 = allbd.getY_soi2a().getRhtot();
			y2=0;
			if (cd.getM_veg().getVegcov()>0.) {
					y2 += Double.valueOf((String)targetTB.getValueAt(Configurer.I_NPPt, 1));  // Soil heteorotrophic res. target is NPP  
			}
			...
			double vegc[] = new double[DATA.ConstCohort.NUM_PFT_PART];
			vegc = allbd.getY_vegs().getC();
			y1 = vegc[0];
		    y2 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_VEGCLt, 1));	 
			y3 = y2;
			var1plotter.vegclTP.addPoint(x, y1, y2);		
			
			y1 = vegc[1];
		    y2 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_VEGCSt, 1));	 
			y3 += y2;
			var1plotter.vegcsTP.addPoint(x, y1, y2);
					
			y1 = vegc[2];
		    y2 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_VEGCRt, 1));	 
			y3 += y2;
			var1plotter.vegcrTP.addPoint(x, y1, y2);

			y1 = allbd.getY_vegs().getCall();
			var1plotter.vegcTP.addPoint(x, y1, y3);
			
			//
			y1 = allbd.getY_vegs().getStrn()[0];
		    y2 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_VEGNLt, 1));	 
			y3 = y2;
			var1plotter.vegnlTP.addPoint(x, y1, y2);		
			
			y1 = allbd.getY_vegs().getStrn()[1];
		    y2 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_VEGNSt, 1));	 
			y3 += y2;
			var1plotter.vegnsTP.addPoint(x, y1, y2);
					
			y1 = allbd.getY_vegs().getStrn()[2];
		    y2 = Double.valueOf((String)targetTB.getValueAt(Configurer.I_VEGNRt, 1));	 
			y3 += y2;
			var1plotter.vegnrTP.addPoint(x, y1, y2);

			y1 = allbd.getY_vegs().getNall();
	...
	public void updateMlyBioGraph(int yrcnt, int im, BgcData allbd){		//update plots
		if(BioVariablePlotter.f.isVisible()){
			...
			y1 = allbd.getM_a2v().getGppall();
			y2 = allbd.getM_a2v().getNppall();
			y3 = allbd.getM_v2soi().getLtrfalcall();
			var1plotter.m_vcfluxTP.addPoint(x, y1, y2, y3);

			y1 = allbd.getM_vegs().getC()[0];
			var1plotter.m_vegclTP.addPoint(x, y1);		
			
			y1 = allbd.getM_vegs().getC()[1];
			var1plotter.m_vegcsTP.addPoint(x, y1);		
				
			y1 = allbd.getM_vegs().getC()[2];
			...
			y1 = allbd.getM_vegs().getLabn();
			y2 = allbd.getM_vegs().getStrn()[0];
			var1plotter.m_vegnlTP.addPoint(x, y1, y2);		
			
			y1 = allbd.getM_vegs().getStrn()[1];
			var1plotter.m_vegnsTP.addPoint(x, y1);		
				
			y1 = allbd.getM_vegs().getStrn()[2];
