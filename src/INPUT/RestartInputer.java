...
public class RestartInputer {
	...
	public void getRestartData(DataRestart resid, int cid) { 
		...
		resid.ysf = ysfA.getInt(ind4.set(cid));
			...
			resid.ifwoody = ifwoodyA.getInt(ind5.set(cid));
		
			Index ind6 = ifdeciwoodyA.getIndex(); 
			resid.ifdeciwoody = ifdeciwoodyA.getInt(ind6.set(cid));
		
			Index ind7 = ifperenialA.getIndex(); 
			resid.ifperenial = ifperenialA.getInt(ind7.set(cid));
		
			Index ind8 = ifvascularA.getIndex(); 
			resid.ifvascular = ifvascularA.getInt(ind8.set(cid));
		
			Index ind9 = vegageA.getIndex(); 
			resid.vegage = vegageA.getInt(ind9.set(cid));
		
			Index ind10 = vegcovA.getIndex(); 
			resid.vegcov = vegcovA.getDouble(ind10.set(cid));
		
			Index ind11 = laiA.getIndex(); 
			resid.lai = laiA.getDouble(ind11.set(cid));
		
			Index ind12 = rootfracA.getIndex(); 
			for (int i=0; i<ConstLayer.MAX_ROC_LAY; i++){
				resid.rootfrac[i] = rootfracA.getDouble(ind12.set(cid, i));
			}
			
			Index ind13 = vegwaterA.getIndex(); 
			resid.vegwater = vegwaterA.getDouble(ind13.set(cid));
		
			Index ind14 = vegsnowA.getIndex(); 
			resid.vegsnow = vegsnowA.getDouble(ind14.set(cid));

			Index ind15 = vegcA.getIndex(); 
			for (int i=0; i<ConstCohort.NUM_PFT_PART; i++){
				resid.vegc[i] = vegcA.getDouble(ind15.set(cid, i));
			}
			
			Index ind16 = labnA.getIndex(); 
			resid.labn = labnA.getDouble(ind16.set(cid));
		
			Index ind17 = strnA.getIndex(); 
			for (int i=0; i<ConstCohort.NUM_PFT_PART; i++){
				resid.strn[i] = strnA.getDouble(ind17.set(cid, i));
			}
			
			Index ind18 = deadcA.getIndex(); 
			resid.deadc = deadcA.getDouble(ind18.set(cid));
		
			Index ind19 = deadnA.getIndex(); 
			resid.deadn = deadnA.getDouble(ind19.set(cid));
		
			Index ind20 = unnormleafA.getIndex(); 
			resid.unnormleaf = unnormleafA.getDouble(ind20.set(cid));

			Index ind21 = toptaA.getIndex();
			for (int i=0; i<10; i++){
				resid.toptA[i] = toptaA.getDouble(ind21.set(cid, i));
			}
			
			Index ind22 = eetmxaA.getIndex(); 
			for (int i=0; i<10; i++){
				resid.eetmxA[i] = eetmxaA.getDouble(ind22.set(cid, i));
			}
			
			Index ind23 = petmxaA.getIndex(); 
			for (int i=0; i<10; i++){
				resid.petmxA[i] = petmxaA.getDouble(ind23.set(cid, i));
			}
			
			Index ind24 = unnormleafmxaA.getIndex(); 
			for (int i=0; i<10; i++){
				resid.unnormleafmxA[i] = unnormleafmxaA.getDouble(ind24.set(cid, i));
			}
			
			Index ind25 = prvfoliagemxaA.getIndex(); 
			resid.prvfoliagemxA = prvfoliagemxaA.getDouble(ind25.set(cid));
